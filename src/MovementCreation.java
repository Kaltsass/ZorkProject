...
public class MovementCreation {
    ...
    static {
        rooms[0] = new RoomsAttempt('F'); // Room 1δ
        rooms[1] = new RoomsAttempt('U'); // Room 2
        rooms[2] = new RoomsAttempt('N'); // Room 3
        rooms[3] = new RoomsAttempt('K'); // Room 4
        rooms[4] = new RoomsAttempt('O'); // Room 5
    }
    ...
    private static void goToNextRoom() {
        if (currentRoomIndex < TOTAL_ROOMS - 1) {
            ...
            rooms[currentRoomIndex].displayRoomLetter(); // Εμφάνιση του γράμματος του δωματίου
        } else {
